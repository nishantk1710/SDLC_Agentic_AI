...
def build_refactor_graph():
    """Build the standalone refactoring graph: apply a code-review report's findings.

    A single-node graph (START → refactoring → END). Separate from the IMP-001 code-gen loop so it
    can run on its own from a report (see ``POST /implementation/refactor``) without touching the
    code-generation path. No checkpointer: there is no interrupt here, so ``invoke`` needs no
    thread id. When a review stage that produces ``review_report`` is wired in, ``refactoring_node``
    can also be slotted into the main pipeline.
    """
    graph = StateGraph(WorkflowState)
    graph.add_node("refactoring", refactoring_node)
    graph.add_edge(START, "refactoring")
    graph.add_edge("refactoring", END)
    return graph.compile()


refactor_workflow = build_refactor_graph()
